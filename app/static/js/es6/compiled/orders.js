(function() {
  ...
  function updateTotal() {
    var subtotalCosts = $('.menu-item').map((function(i, d) {
      return $(d).data('subtotalCost');
    })).toArray();
    var subtotalCals = $('.menu-item').map((function(i, d) {
      return $(d).data('subtotalCalories');
    })).toArray();
    var totalCost = 0;
    var totalCalories = 0;
    if (subtotalCosts.length) {
      totalCost = subtotalCosts.reduce((function(p, c) {
        return p + c;
      }));
    }
    if (subtotalCals.length) {
      totalCalories = subtotalCals.reduce((function(p, c) {
        return p + c;
      }));
    }
